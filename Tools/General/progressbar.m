function [status] = progressbar(varargin)
% Description:
...
%   - Randomized colour. When a programmer gets bored...
%
...
%
% Example Function Calls For Querying The Status and Changing Active Labels
%   status = progressbar('status')      % Returns a struct "status" containing {progfig, progdata, lastupdate}
%
%   progressbar('changelabel',4,'Processing session 30140902xyt01') % Changes the fourth label of an active progressbar figure.
%
% Notes:
...
%   Bar colour is chosen randomly when the figure is created or reset. Clicking
% the bar will cause a random colour change.
%
% Demos:
%     % Single bar
%     m = 500;
...
% 2004-Oct-08   Separate function for timeleftstr, expand to include days
...
% 
% Revised 2015-Nov-19 by Gwylan Scheeren to incorporate ability to query
% status and ability to change the labels of active progressbar figures
%
...
% return progressbar status or change label if required
labelchanged = 0;
if strcmp(input{1},'status')
    status.progfig = progfig;
    status.progdata = progdata;
    status.lastupdate = lastupdate;
    
    % Update bar label if needed
elseif strcmp(input{1},'changelabel') && ~(input{2} > numel(progdata)) && isfield(progdata(1).proglabel,'String')
    set(progdata(input{2}).proglabel,'String',input{3});
    labelchanged = 1;
    drawnow
    ...
    if ishandle(progfig)
        ...
        for ndx = 1:nbars
            ...
            % Set callbacks to change colour on mouse click
            ...
            % Pick a random colour for this patch
